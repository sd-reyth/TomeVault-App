  ...
  return (
    <div className="tv-backdrop fixed inset-0 z-50 flex items-end justify-center p-2 backdrop-blur-md sm:items-center sm:p-4">
      <div className={`tv-surface tv-text relative flex max-h-[calc(100dvh-1rem)] w-full ${maxWidthClassName} flex-col overflow-hidden rounded-[1.5rem] transition-all duration-200 ease-out sm:max-h-[calc(100dvh-2rem)] sm:rounded-[1.5rem] animate-in fade-in zoom-in-95 slide-in-from-bottom-4 sm:slide-in-from-center-0 ${panelClassName}`}>
        <div className="tv-backdrop pointer-events-none absolute inset-x-0 top-0 h-32 blur-[42px]" />

        <div className="relative z-10 flex shrink-0 items-start justify-between gap-3 border-b border-white/10 px-5 py-5 sm:px-6">
          <div className="min-w-0">
            ...
            {subtitle ? (
              <p className="tv-text-sub mt-2 max-w-md text-sm font-story leading-relaxed">{subtitle}</p>
            ) : null}
          </div>
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Sluiten"
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 transition-all duration-200 ease-out hover:scale-105 active:scale-95"
          >
