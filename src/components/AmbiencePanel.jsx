...
export default function AmbiencePanel({
  ...
  return (
    <div className="absolute right-0 top-full z-50 mt-3 w-[min(92vw,31rem)] overflow-hidden rounded-[1.6rem] border border-stone-700/80 bg-stone-950/95 shadow-[0_28px_80px_rgba(0,0,0,0.52)] backdrop-blur-xl">
      <div className="border-b border-stone-800/80 bg-gradient-to-r from-amber-900/30 via-stone-950 to-stone-950 px-4 py-4 md:px-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-full border border-stone-800 bg-stone-900/80 text-stone-400 transition-colors hover:border-stone-700 hover:text-stone-200"
        ...
        <div className="mt-4 rounded-2xl border border-stone-800/80 bg-stone-900/70 p-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border px-4 py-3 font-fantasy text-sm tracking-[0.14em] transition-colors ${isPlaying ? 'border-amber-700/50 bg-amber-950/40 text-amber-100 hover:bg-amber-900/45' : 'border-stone-700 bg-stone-950 text-stone-200 hover:border-amber-700/40 hover:text-amber-100'}`}
            ...
            {verifiedTracks.map((track) => {
              ...
              return (
                <button
                  ...
                  disabled={!canControlSession}
                  className={`group relative overflow-hidden rounded-[1.35rem] border p-4 text-left transition-all ${isActiveTrack ? 'border-amber-600/50 bg-stone-900 shadow-[0_18px_50px_rgba(245,158,11,0.1)]' : 'border-stone-800/80 bg-stone-950/80 hover:border-stone-700'} ${!canControlSession ? 'cursor-default' : ''}`}
                >
                  ...
                  <div className="relative flex items-start justify-between gap-4">
                    ...
                    <div className={`shrink-0 rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] ${isActiveTrack ? 'border-amber-400/40 bg-amber-200/10 text-amber-100' : 'border-stone-700 bg-stone-950/80 text-stone-400'}`}>
              ...
              <div key={`${track.id}-credit`} className="rounded-2xl border border-stone-800 bg-stone-900/70 px-4 py-3">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-stone-700 bg-stone-950 px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-stone-200 transition-colors hover:border-amber-600/40 hover:text-amber-100"
                  >
                    Open bron
